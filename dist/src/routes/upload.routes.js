...
exports.uploadChunk = exports.uploadThumbnailOnly = exports.uploadVideoOnly = exports.uploadLesson = void 0;
...
const uploadLesson = async (req, res) => {
    try {
        ...
        const videoUrl = getRelativePath(files.video[0]);
        const thumbnailUrl = files.thumbnail?.[0] ? getRelativePath(files.thumbnail[0]) : undefined;
        const homeworkPdfUrl = files.pdf?.[0] ? getRelativePath(files.pdf[0]) : undefined;
        console.log('Saved Video to DB:', videoUrl);
        if (thumbnailUrl)
            console.log('Saved Thumbnail to DB:', thumbnailUrl);
        if (homeworkPdfUrl)
            console.log('Saved Homework PDF to DB:', homeworkPdfUrl);
...
const VIDEOS_DIR = path_1.default.join(UPLOADS_DIR, 'videos');
const PDFS_DIR = path_1.default.join(UPLOADS_DIR, 'pdfs');
const TEMP_DIR = path_1.default.join(UPLOADS_DIR, 'temp');
const COURSE_THUMBNAILS_DIR = path_1.default.join(UPLOADS_DIR, 'thumbnails', 'course-thumbnails');
const LESSON_THUMBNAILS_DIR = path_1.default.join(UPLOADS_DIR, 'thumbnails', 'lesson-thumbnails');
...
const uploadVideoOnly = async (req, res) => {
    try {
        ...
        const videoUrl = getRelativePath(videoFile);
...
exports.uploadVideoOnly = uploadVideoOnly;
// ─── Standalone Thumbnail Upload Endpoint ─────────────────────────────────
const uploadThumbnailOnly = async (req, res) => {
    try {
        const authReq = req;
        const authUser = authReq.user;
        ...
        const thumbnailUrl = getRelativePath(thumbnailFile);
...
exports.uploadThumbnailOnly = uploadThumbnailOnly;
// ─── Route for standalone thumbnail upload ─────────────────────────────────
router.post('/thumbnail', auth_middleware_1.authMiddleware, role_middleware_1.checkTeacherApproval, upload.single('thumbnail'), exports.uploadThumbnailOnly);
// ─── Route for standalone video upload ──────────────────────────────────
router.post('/video', auth_middleware_1.authMiddleware, role_middleware_1.checkTeacherApproval, upload.single('video'), exports.uploadVideoOnly);
// ─── Route ────────────────────────────────────────────────────────────────────
// CRITICAL ORDER:
...
router.post('/', auth_middleware_1.authMiddleware, role_middleware_1.checkTeacherApproval, upload.fields([
    { name: 'video', maxCount: 1 },
    { name: 'thumbnail', maxCount: 1 },
    { name: 'pdf', maxCount: 1 },
]), exports.uploadLesson);
...
const uploadChunk = async (req, res) => {
    try {
        ...
        const tempFilePath = path_1.default.join(TEMP_DIR, tempFileName);
        if (!req.file) {
            res.status(400).json({ error: 'Chunk file is required' });
            return;
        }
        const chunkBuffer = fs_1.default.readFileSync(req.file.path);
        if (chunkIndexNum === 0) {
            fs_1.default.writeFileSync(tempFilePath, chunkBuffer);
        }
        else {
            fs_1.default.appendFileSync(tempFilePath, chunkBuffer);
        }
        fs_1.default.unlinkSync(req.file.path);
        if (chunkIndexNum === totalChunksNum - 1) {
            const finalFileName = `${Date.now()}-${fileName.replace(/[^a-zA-Z0-9._-]/g, '_')}`;
            const finalFilePath = path_1.default.join(VIDEOS_DIR, finalFileName);
...
router.post('/chunk', (0, multer_1.default)({
    storage: multer_1.default.diskStorage({ destination: TEMP_DIR }),
    limits: { fileSize: 10 * 1024 * 1024 } // 10MB max per chunk
}).single('chunk'), exports.uploadChunk);
